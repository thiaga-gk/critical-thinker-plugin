...
- **Fallback map.** Every frontmatter `model:` is the *preferred* model. When it cannot run, apply the ordered per-role fallback in `references/model-map.md` as a runtime model override and record the actual model that ran; the pins never change. Two invariants hold: never drop a role below its tier (never Haiku, `inherit`, or below Sonnet), and the cross-model reviewer stays a different family than the certifier.
...
- The certifier prefers `claude-opus-4-8` and may fall back **within the Opus tier** (`claude-opus-4-7` → `claude-opus-4-6` → `claude-opus-4-5`), recording the actual version (e.g. `VALIDATED (certifier: claude-opus-4-7)`). Only when **no Opus-tier model** can run, report `VALIDATION BLOCKED — OPUS UNAVAILABLE`; never certify with Sonnet, Fable, or an unauditable model. The cross-model reviewer prefers Fable 5 and falls back to Sonnet 5 (never Opus); record `MODEL DIVERSITY UNAVAILABLE` only if neither can run.
